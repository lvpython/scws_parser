...
static void init_scws(){
  ...
  dict_path = "/Users/lvpython/packages/pg_chinese/zhparser/dict.utf8.xdb";
  if(dict_in_memory)
...
void scwsprs_start(char *content)
{
  ParserState *pst = (ParserState *) malloc(sizeof(ParserState));
  char s_type[4] = {0};
  int type = -1;
  pst -> buffer = (char *) content;
  pst -> len = strlen(content);
  pst -> pos = 0;
  ...
  while (((pst -> res) = (pst -> curr) = scws_get_result(pst -> scws)) != NULL)
  {
    while (pst -> curr != NULL)
    {
      printf( "%.*s", pst -> curr->len, pst -> buffer + pst -> curr->off);
      if (pst -> curr->len != 1 || ((*(pst -> buffer + pst -> curr->off) != '\n')
        && (*(pst -> buffer + pst -> curr->off) != '\r')))
      {

        sprintf(s_type,"%.3s", pst -> curr->attr);
        type = get_lextype(s_type);
        printf("|%s|%d",s_type, type);
      }
      pst -> curr = pst -> curr->next;
    }
...
{
  char *text1= "革云士净化器";
  char *text2= "所谓自己约得私有化，含着泪也要走完。这显然是陌陌目前不想做的。首先，A股目前已暂停IPO，即便私有化，上市也是一个漫长的过程。其次，目前形势还不明朗，国泰君安上市七天便遭遇四天下跌，互联网公司暴风科技已经停牌，乐视网也大幅下跌。陌陌、360等公司现在回归还将面临遥遥无期的排队以及十分不确定的前景，所以恐怕不会很着急私有化。";
  init_scws();
   // for(int i = 0; i < sizeof(descr)/sizeof(descr[0]); i++)
   // {
   //   printf("%.3s", descr[i].alias);
   // }
  scwsprs_start(text1);
  // int i = 0;
  // int j = 0;
